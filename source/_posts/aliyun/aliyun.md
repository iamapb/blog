---
title: aliyun 发送短信后 上行消息进行接收
date: 2021-02-03 13:53:06
tags: aliyun
---


> 1. 先在阿里云短信服务系统设置里面勾选上行数据接收 

![image](/../../static/aliyun/aliyun1.png)

> 2. 查看短发服务 短信回执文档 有对应的demo可以下载 
...
> 4. 编写一个方法 随着主程序的启动而启动 实时监听短信的回执回复
```java

    @Component
    class MyCrmApplicationRunner: ApplicationRunner {
    
        @Autowired
        lateinit var aliyunTemplate: AliyunMessageTemplateTask
    
        override fun run(args: ApplicationArguments?) {
            // 调用上面的main里面的方法 可以自己封装成一个方法
        }
    }
```
> 5. 短信的上行消息接收所用的jar包 maven无法获取 只能够手动导入到lib目录中 具体操作可以看
我maven引入外部jar包
